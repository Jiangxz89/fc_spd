...
<body>
	<div class="right-main-box"> 
		...
		<div class="searchBox">
			<form:form id="searchForm" modelAttribute="medstoYptyd" action="${ctx}/pd/medstoYptyd/findDrugReturnBillDetaile" method="post" class="breadcrumb form-search">
					...
					<tbody>
						<c:forEach items="${page.list }" var="drug">
							<tr>
								...
								<td>${drug.inventorystorage}</td>
								<td>${drug.storagenumber}</td>
								<td>${drug.inventorynumber}</td>
								<td><fmt:formatDate value="${drug.thrq}" pattern="yyyy-MM-dd"/></td>
								<td>${drug.thyy}</td>
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#exportBtn').one('click',function(){
				...
				form.attr('action', '${ctx}/excelExport/drugReturnBillDetaileExport');
				var input = $('<input>');
