	...
	<div class="right-main-box">
		<form:form id="inputForm" style="padding:0;" modelAttribute="pdDosageDetail" action="${ctx}/pd/pdDosageDetail/saveCharge" method="post" class="form-horizontal">
			...
			<div class="addRoomBox">
				...
				<input type="text" /> -->
				<input type="hidden" name="isCharge" value="2"/><!-- 取消收费为2 -->
			...
			<div class="tableBox">
				<table class="hcy-table-gray">
					<thead>
						<tr>
							<th><input type="checkbox" id="allchoose"/>取消收费</th>
							<th>产品名称</th>
							<th>产品编号</th>
							<th>产品条码</th>
							<th>规格</th>
				...
				<div class="totalText">
					...
					<%-- <span id="allNum">取消收费产品总数:<span class="all_count">${resultMap['p_num'] }</span></span>
					<span id="allMoney">总金额:<span class="all_money">${resultMap['p_money'] }</span></span> --%>
					<span id="allNum">取消收费产品总数:<span class="all_count">0</span></span>
	...
	<script type="text/javascript">
		$(function(){
            ...
            if(!detailListSize ||detailListSize==0){
                layer.alert('产品还没有进行收费或已经全部退费了',{icon:0});
            }
			...
			$('#submitbtn').click(function(){
				...
				if(!cont){
					layer.alert('还没有取消收费的产品',{icon:0});
				}else if(obj.length < 1){
					layer.alert('请选择取消收费的产品',{icon:0});
				}else{
					var ids = [];
