...
<body>
	<div class="right-main-box">
		...
		<div class="newSearchBox">
			<form:form id="searchForm" modelAttribute="pdRejectedListHead" action="${ctx}/pd/pdRejectedListHead/pdHospitaloutsideReturnDetail" method="post" class="breadcrumb form-search">
				...
				<c:when test="${user.storeroomType==1}">
					<form:select path="rejectedRepoId" class="input-medium">
				...
				<form:input path="productNumber" autocomplete="off" value="${pdStockRecord.productNumber}"/>
				<label>产品名称</label>
				...
				<br>
				<label>产品型号 </label>
				<form:input path="version" autocomplete="off" style="width:165px;" value="${pdRejectedListHead.version}"/>
				<label>产品批号</label>
				<form:input path="batchNo" autocomplete="off" value="${pdRejectedListHead.batchNo}"/>
	...
	<sys:message content="${message}"/>
		...
		<table id="contentTable" class="hcy-public-table" style="padding:0;">
			...
			<c:forEach items="${page.list}" var="pdRejectedListHead">
				<tr>
					<td>
						${pdRejectedListHead.number} 
					</td>
					<td>
						${pdRejectedListHead.rejectedDate}
					</td>
					<td>
						${spd:getStoreroomName(pdRejectedListHead.rejectedRepoId)}
					</td>
					<td>
						${pdRejectedListHead.ghdwMc}
					</td>
					<td>
						${pdRejectedListHead.productName}
					</td>
					<td>
						${pdRejectedListHead.productNumber}
					</td>
					<td>${pdRejectedListHead.spec }</td>
					<td>${pdRejectedListHead.version}</td>
					<td>${pdRejectedListHead.batchNo}</td>
					<td><fmt:formatDate value="${pdRejectedListHead.validDate}" pattern="yyyy-MM-dd"/></td>
					<td>${pdRejectedListHead.rejectedCount}</td>
					<td>
						${pdRejectedListHead.operPerson}
					</td>
					<td><a href="###" title="${pdRejectedListHead.remarks}" class="overLook">${pdRejectedListHead.remarks}</a></td>					
				</tr>
			</c:forEach>
			</tbody>
		</table>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(".hcy-reset").click(function(){
                ...
                $("#rejectedRepoId").select2("val", sel);
			...
			$('#exportBtn').one('click',function(){
				...
				form.attr('action', '${ctx}/excelExport/hospitaloutsideReturnDetailExport');
				var input = $('<input>');
