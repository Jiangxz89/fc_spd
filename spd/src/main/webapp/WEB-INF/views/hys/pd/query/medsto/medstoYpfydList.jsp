...
<body>
	<div class="right-main-box"> 
		...
		<div class="searchBox">
			<form:form id="searchForm" modelAttribute="medstoYpfyd" action="${ctx}/pd/medstoYpfyd/findDispensingListDetail" method="post" class="breadcrumb form-search">
					...
					<tbody>
						<c:forEach items="${page.list }" var="drug">
							<tr>
								...
								<td>${drug.patientage}</td>
								<td>${drug.patientsex}</td>
								<td>${drug.number}</td>
								<td>${drug.patientid}</td>
								<td>${drug.outpatientid}</td>
								<td>${drug.outpatientdoctor}</td>
								<td>${drug.pharmacist}</td>
								<td><fmt:formatDate value="${drug.fydatetime}" pattern="yyyy-MM-dd"/></td>
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#exportBtn').one('click',function(){
				...
				form.attr('action', '${ctx}/excelExport/dispensingListDetailExport');
				var input = $('<input>');
