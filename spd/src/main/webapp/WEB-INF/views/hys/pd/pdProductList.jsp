...
<body>
	...
	<div class="right-main-box">
		...
		<form:form id="searchForm" style="padding:0;background:#fff;" modelAttribute="pdProduct" action="${ctx}/pd/pdProduct" method="post" class="breadcrumb form-search">
					...
					<form:form id="dataForm" modelAttribute="productExcelList" method="post" class="breadcrumb form-search">
						<c:forEach items="${page.list }" var="pdProduct" varStatus="a">
							<tr>
								...
								<td><input type="hidden" name="list[${a.index }].number" value="${pdProduct.number }" /><a href="${ctx }/pd/pdProduct/toUpdate?id=${pdProduct.id}&flag=see" title="${pdProduct.number }" >${pdProduct.number }</a></td>
								<td><input type="hidden" name="list[${a.index }].name" value="${pdProduct.name }" /><a href="${ctx }/pd/pdProduct/toUpdate?id=${pdProduct.id}&flag=see" title="${pdProduct.name }" >${pdProduct.name }</a></td>
								<td><input type="hidden" name="list[${a.index }].name" value="${pdProduct.otherName }" />${pdProduct.otherName }</td>
								<td><input type="hidden" name="list[${a.index }].category" value="${pdProduct.categoryName }" />${pdProduct.categoryNameShow}</td>
								<td><input type="hidden" name="list[${a.index }].group" value="${pdProduct.groupName }" />${pdProduct.groupNameShow}</td>
								<td><input type="hidden" name="list[${a.index }].unit" value="${pdProduct.unit }" />${pdProduct.unit }</td>
								<td><input type="hidden" name="list[${a.index }].spec" value="${pdProduct.spec }" />${pdProduct.spec }</td>
								<td><input type="hidden" name="list[${a.index }].version" value="${pdProduct.version }" />${pdProduct.version }</td>
								<td><input type="hidden" name="list[${a.index }].vender" value="${pdProduct.venderName }" /><a href="${ctx}/pd/pdVender/form?id=${pdProduct.venderId}&flag=see" title="${pdProduct.venderName }" class="overLook">${pdProduct.venderNameShow}</a></td>
								<td><a href="${ctx}/pd/pdSupplier/form?id=${pdProduct.supplierId}&flag=see" title="${pdProduct.supplierNameShow }" class="overLook">${pdProduct.supplierNameShow }</a></td>
								...
								<td class="chargeCodeTd">${pdProduct.chargeCode }</td><input type="hidden" name="list[${a.index }].chargeCode" value="${pdProduct.chargeCode }" />
								<td><a href="###" title="${pdProduct.registration }" class="overLook"><input type="hidden" name="list[${a.index }].registration" value="${pdProduct.registration }" />${pdProduct.registration }</a></td>
								<td><input type="hidden" name="list[${a.index }].sellingPrice" value="${pdProduct.sellingPrice }" />${pdProduct.sellingPrice }</td>
								<td><input type="hidden" name="list[${a.index }].description" value="${pdProduct.description }" /><a href="###" class="overLook" title="${pdProduct.description }">${pdProduct.description }</a></td>
								<td style="width:120px;">
								<a href="${ctx }/pd/pdProduct/toUpdate?id=${pdProduct.id}&flag=see" class="hcy-btn hcy-btn-o">查看</a>
								<a href="${ctx }/pd/pdProduct/toUpdate?id=${pdProduct.id}&flag=update" class="hcy-btn hcy-btn-o">修改</a>
