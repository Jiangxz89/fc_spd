...
<body>
	<div class="right-main-box"> 
		...
		<div class="newSearchBox">
			<form:form id="searchForm" modelAttribute="pdStockRecord" action="${ctx}/pd/pdStockRecord/stockRecordCallInQuery" method="post" class="breadcrumb form-search">
		...
		<div class="tableBox" style="width:100%;overflow:auto;">
			<table id="contentTable" class="hcy-public-table" style="padding:0;width:1700px;">
				<thead>
					<tr>
						<th>出库单号</th>
						<th>出库日期</th>
				...
				<tbody>
					<c:forEach items="${page.list }" var="pdStockRecord">
						<tr>
							<td>${pdStockRecord.outReCordNo }</td>
							<td><fmt:formatDate value="${pdStockRecord.recordDate}" pattern="yyyy-MM-dd"/></td>
							<td>${spd:getStoreroomName(pdStockRecord.outId)}</td>
							<td>${spd:getStoreroomName(pdStockRecord.inId)}</td>
							<td>${pdStockRecord.pdName}</td>
							<td>${pdStockRecord.number}</td>
							<td>${pdStockRecord.productBarCode}</td>
							...
							<td>${pdStockRecord.recordPeople }</td>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('#exportBtn').one('click',function(){
				...
				form.attr('action', '${ctx}/excelExport/stockRecordcalledInQueryExport');
				var input = $('<input>');
