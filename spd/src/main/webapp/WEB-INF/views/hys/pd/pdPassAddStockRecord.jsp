...
<body>
	<div class="right-main-box">
		...
		<div class="searchBox">
			...
			<input id="allocationNo" name="allocationNo" value="${pdStockRecord.allocationNo}" type="hidden"/>
			<input id="applyNo" name="applyNo" value="${pdStockRecord.applyNo}" type="hidden"/>
			<input id="rowsLen" value="0" type="hidden"/>			
				...
				<div>
					<label>出库日期</label>
					<input name="recordDate" type="text" readonly="readonly" maxlength="100" id="recordDate" class="input-xlarge"
						value="<fmt:formatDate value="${pdStockRecord.recordDate}" pattern="yyyy-MM-dd"/>"/>					
				</div>		
				<div>
					<label>出库类型 </label>
					<input type="text" value="${fns:getDictLabel(pdStockRecord.outType, 'stock_out_type', '')}" readonly="readonly"/>
					<input id="outType" name="outType" type="hidden" value="${pdStockRecord.outType}"/>
				</div>
				<div>
					<label>操作人</label>
				...
				<div>
					...
					<input type="hidden" name="outId" id="outId" value="${pdStockRecord.outId }"/>
				</div>
				<div>
					<label for="">入库库房</label>
					<form:input path="inName" htmlEscape="false" maxlength="64" class="input-medium" value="${pdStockRecord.inName}" readonly="readonly"/>		
					<input type="hidden" id="inId" name="inId" value="${pdStockRecord.inId }"/>
				</div>				
				<br>						
		...
		<div class="tableBox">
			...
			<div class="form-actions" style="background:#fff;border:none;text-align:center;padding:0;margin-top:30px;">
				...
				<input id="btnCancel" class="hcy-btn hcy-back" type="button" value="返回" />
			</div>
		</form:form>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			var allocationCode=$("#allocationNo").val();
			var applyNo=$("#applyNo").val();
			if(allocationCode){allocationOut();}
			if(applyNo){applyOut()}
			function allocationOut(){
				$.getJSON('${ctx}/pd/pdAllocationRecord/getData',{allocationCode:allocationCode},function(data){
					var datap = data.productList, data = data.packageList;
					var htmlRef='';					
					//定数包
	...
	$(document).ready(function() {
		...
		function getProdHtmlOne(data){
			...
			for(var i=0; i<data.length; i++){
				...
				html +=		'<td><input type="text" name="productList['+len+'].productNum" class="inputRejectedCount" value="1" style="width:40px;"/>';
	...
	$("#btnCancel").click(function(){
		...
		var flag='${flag}';
		if(flag=='0'){//调拨
			window.location.href="${ctx}/pd/pdAllocationRecord/examineList";
		}else if(flag="1"){
