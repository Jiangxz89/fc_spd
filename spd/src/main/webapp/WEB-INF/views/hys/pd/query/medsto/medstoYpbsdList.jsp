...
<body>
	<div class="right-main-box"> 
		...
		<div class="searchBox">
			<form:form id="searchForm" modelAttribute="medstoYpbsd" action="${ctx}/pd/medstoYpbsd/findDrugLostOrderDetaile" method="post" class="breadcrumb form-search">
					...
					<tbody>
						<c:forEach items="${page.list }" var="drug">
							<tr>
								...
								<td>${drug.damagenumber}</td>
								<td><fmt:formatDate value="${drug.damagedatetime}" pattern="yyyy-MM-dd"/></td>
								<td>${drug.damageremark}</td>
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#exportBtn').one('click',function(){
				...
				form.attr('action', '${ctx}/excelExport/drugLostOrderDetaileExport');
				var input = $('<input>');
