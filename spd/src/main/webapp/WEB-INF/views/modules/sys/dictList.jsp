			...
			<c:forEach items="${page.list}" var="dict">
				<tr>
					...
					<shiro:hasPermission name="sys:dict:edit"><td>
			  				<a class="hcy-btn hcy-btn-o" href="${ctx}/sys/dict/form?id=${dict.id}">修改</a>
						<a class="hcy-btn hcy-btn-o" href="${ctx}/sys/dict/delete?id=${dict.id}&type=${dict.type}" onclick="return confirmx('确认要删除该字典吗？', this.href)">删除</a>
			  				<a class="hcy-btn hcy-btn-o" href="<c:url value='${fns:getAdminPath()}/sys/dict/form?type=${dict.type}&sort=${dict.sort+10}'><c:param name='description' value='${dict.description}'/></c:url>">添加键值</a>
