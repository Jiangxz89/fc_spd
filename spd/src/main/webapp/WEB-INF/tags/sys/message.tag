...
<c:if test="${not empty content}">
	...
	<%--<div id="messageBox" class="alert alert-${ctype} hide">--%><%--2019年5月20日14:48:41 提示弹窗修改--%>
	<div id="messageBox" class="alert alert-${ctype}">
		<button data-dismiss="alert" class="close">×</button>${content}
	</div>
	<script type="text/javascript">
		if(!top.$.jBox.tip.mess){top.$.jBox.tip.mess=1;top.$.jBox.tip("${content}","${ctype}",{persistent:true,opacity:0});$("#messageBox").show();}</script>
</c:if>