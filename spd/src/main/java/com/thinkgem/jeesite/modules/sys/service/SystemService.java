...
import static com.thinkgem.jeesite.modules.sys.utils.UserUtils.CACHE_ROLE_LIST;
import static com.thinkgem.jeesite.modules.sys.utils.UserUtils.putCache;
...
public class SystemService extends BaseService implements InitializingBean {
	 ...
	 * @return
	 */
	public List<User> finUserByLoginName(User user) {
		return UserUtils.findByLoginName(user);
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void updatePasswordById(String id, String loginName, String mobile, String newPassword) {
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 * @return
	 */
	public User loginUserPwd( User user ) {
		return userDao.getByLoginNamePwd( user );
	}
	
	
	/**
	 * 添加用户 
	 * loginType=front 添加前段用户
	 * loginType=app 添加代理用户
	 * loginType=app 添加app用户
	 * @param user
	 * @param loginType
	 */
	@Transactional(readOnly = false)
	public void saveUser(User user,String  loginType) {
	...
	public  User getFrontUser(User user) {
	...
	public  boolean existUserByMobile(User user) {
		if(StringUtils.isBlank( user.getMobile( ) )){
			return true;
		}
		List<User>  list=userDao.getUserByModel(user);
	...
	public  User getAdminUser(User user) {
		if(StringUtils.isBlank( user.getMobile( ) ) && StringUtils.isBlank( user.getLoginName( ) )){
			return null;
		}
		List<User>  userList = userDao.findList( user );
		if(userList==null || userList.size( )==0){
	...
	/**
	 * 是否存在admin 用户 通过手机号或登录名
	 * @param user
	 * @return
	 */
	public  boolean existAdminUser(User user) {
		return getAdminUser(user)!=null?true:false;
	}
	 ...
	 * @return
	 */
	public List<String> getMobileByUserId(List<String> userIdList){
		...
		List<User> userList=userDao.getMobileByUserId(user);
		if(userList!=null && userList.size( )>0){
			for(User u :userList){
				list.add( u.getMobile( ) );
			}
		}
	 return list;
	}
	 ...
	 * @return
	 */
	public List<Role> findUpRoleList(){
		return UserUtils.findUpRoleList();
	}
	 ...
	 * @return
	 */
	public Page<User> newFindList(Page<User> page, User user){
		user.setPage(page);
		// 执行分页查询
	 ...
	 * @return
	 */
	public Page<User> getNewUserPage(Page<User> page, User user){
		user.setPage(page);
		page.setList(userDao.getNewUser(user));
		return page;
	}
}
